...
public class Block
{	
	private final int BLOCK_SIZE	= 800;
	...
	public Block( String filename )
	{
		...
		System.out.println("Load block: " + ((fi - ini) / 1000) + " seconds\n");
	}
		...
		throws MatmulAppException 
	{
		...
		{
			throw new MatmulAppException( fnfe.getMessage() );
		}
		catch ( IOException ioe )
		{
			throw new MatmulAppException( ioe.getMessage() );
		}

		fi = System.currentTimeMillis();
		System.out.println("Store block: " + ((fi - ini) / 1000) + " seconds\n");
	}
	...
	public void multiplyAccum ( Block a, Block b )
	{
		...
		for( int i = 0; i < this.bRows; i++ )			// rows
		{
			for( int j = 0; j < this.bCols; j++ )		// cols
			{
                ...
                System.out.println("Multiply raw: " + ((fi - ini) / 1000) + " seconds\n");
