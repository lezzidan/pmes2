...
{	
	...
	public Block( String filename ) throws CholeskyAppException
	{
		bRows = bCols = BLOCK_SIZE;
		data = new double[bRows][bCols];
		try
		...
		throws CholeskyAppException
	{
		...
		for( int j = 0; j < BLOCK_SIZE; j++)
		{
			...
			if(L.data[j][j] <= 0) 
			{
				throw new CholeskyAppException( "Diagonal element L[" + j + "][" + j + "] is negative" );
			}
		...
		throws CholeskyAppException
	{
		...
		for(int j = 0; j < this.bCols; j++)
		{
			...
			if(!found)   
			{
				throw new CholeskyAppException( "We can't find the inverse of this matrix..." );
			}
			swap = I.data[i][j];
