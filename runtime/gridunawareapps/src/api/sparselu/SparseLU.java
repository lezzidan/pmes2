
package api.sparselu;

import integratedtoolkit.api.IntegratedToolkit;
import integratedtoolkit.api.impl.IntegratedToolkitImpl;

import java.io.FileNotFoundException;
import java.io.FileOutputStream;
import java.io.IOException;

import worker.sparselu.Block;
import worker.sparselu.SparseLUAppException;
import worker.sparselu.SparseLUImpl;


public class SparseLU {

	public static int NB;
	
	private static boolean[][] matriu;


	private static void genmat() {
		boolean null_entry;

		for (int i = 0; i < NB; i++) 
			for (int j = 0; j < NB; j++) {
				matriu[i][j] = false;	
				null_entry = false;
				if ((i < j) && (i%3 != 0)) null_entry = true;
				if ((i > j) && (j%3 != 0)) null_entry = true;
				if (i%2 == 1) null_entry = true;
				if (j%2 == 1) null_entry = true;
				if (i == j) null_entry = false;
				if (i == j-1) null_entry = false;
				if (i-1 == j) null_entry = false;
				if (!null_entry)
					matriu[i][j] = true;
			}
	}

	public static void initialize(String filename, int ii, int jj) {
		FileOutputStream fos = null;
		try {
			fos = new FileOutputStream(filename);
		}
		catch (FileNotFoundException fnfe) {
			fnfe.printStackTrace();
			System.exit(1);
		}
			
		try {
			double initVal = 1325;
			for ( int i = 0; i < Block.BLOCK_SIZE; i++ ) {	
				for(int j = 0; j < Block.BLOCK_SIZE; j++) {
					initVal = (3125 * initVal) % 65536;
					double cellValue = 0.0001;
					if (ii == jj) {
						if (i == j)
							cellValue = -20000;
						if ((i - 1 == j) || (i == j - 1))
							cellValue = 10000;
					}
					fos.write((cellValue + " ").getBytes());
				}
				fos.write("\n".getBytes());
			}
			fos.close();
		}
		catch (IOException ioe) {
			ioe.printStackTrace();
			System.exit(1);
		}		
	}

	public static void main(String args[]) {
		String file, file1, file2, file3, file4;

		if ( args.length != 1 ) {
			System.out.println("Usage: java SparseLU <matrix_dimension>\n");
			return;
		}
		
		NB = Integer.parseInt(args[0]);
		
		matriu = new boolean[NB][NB];
		
		genmat();
		
		IntegratedToolkit it = new IntegratedToolkitImpl();	
		it.startIT();
		
		try {
			for (int i = 0; i < NB; i++)
				for (int j = 0; j < NB; j++)
					initialize("A." + i + "." + j, i, j);
				
			for (int k = 0; k < NB; k++) {
				file = "A." + k + "." + k;
				SparseLUImpl.lu0(file);
				for (int j = k+1; j < NB; j++) {
					if (matriu[k][j]) {
						file1 = "A." + k + "." + j;
						SparseLUImpl.fwd(file, file1); 
					}
				}
				for (int i = k+1; i < NB; i++) {
					if (matriu[i][k]) {
						file2 = "A." + i + "." + k;
						SparseLUImpl.bdiv(file, file2);
						for (int j = k+1; j < NB; j++) {
							if (matriu[k][j]) {
								file3 = "A." + k + "." + j;
								file4 = "A." + i + "." + j; 
								if (!matriu[i][j])
									matriu[i][j] = true;
								SparseLUImpl.bmod(file2, file3, file4);
							}
						}
					}
			    }
			}
		}
		catch (SparseLUAppException se) {
			se.printStackTrace();
			return;
		}
		
		it.stopIT(true);
		
	}

}
