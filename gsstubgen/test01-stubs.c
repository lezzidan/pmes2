...
void Subst(file referenceCFG, double seed, double * newCFG)
{
   ...
   extern jclass clsITimpl = NULL; /* integratedtoolkit.api.impl.IntegratedToolkitImpl class */ 
   ...
   midDoubleCon = env->GetMethodID(clsDouble, "<init>", "()V");
   if (env->ExceptionOccurred()) {
   	env->ExceptionDescribe();
   	return 0;
   }
   ...
   env->SetObjectArrayElement(jobjOBJArr, 6, env->NewObject(clsDouble, midDoubleCon, newCFG));
   env->SetObjectArrayElement(jobjOBJArr, 7, jobjParTypeDouble);
   env->SetObjectArrayElement(jobjOBJArr, 8, jobjParDirOUT);

   Execute(SubstOp, 1, 1, 0, 1, referenceCFG, seed, buff_newCFGenv->CallVoidMethod(jobjIT, midExecute,  env->NewStringUTF("Matmul"),  env->NewStringUTF("matmul"), 3, jobjOBJArr);
);

   /* Parameter demarshalling */
   *newCFG = strtod(buff_newCFG, NULL);
...
void Dimem(file newCFG, file traceFile, file DimemasOUT)
{
   ...
   env->SetObjectArrayElement(jobjOBJArr, 5, jobjParDirIN);
   env->SetObjectArrayElement(jobjOBJArr, 6, env->NewStringUTF(DimemasOUT));
   env->SetObjectArrayElement(jobjOBJArr, 7, jobjParTypeFile);
   env->SetObjectArrayElement(jobjOBJArr, 8, jobjParDirOUT);

   Execute(DimemOp, 2, 0, 1, 0, newCFG, traceFile, DimemasOUTenv->CallVoidMethod(jobjIT, midExecute,  env->NewStringUTF("Matmul"),  env->NewStringUTF("matmul"), 3, jobjOBJArr);
);
...
void Post(file newCFG, file DimemasOUT, file FinalOUT)
{
   ...
   env->SetObjectArrayElement(jobjOBJArr, 4, jobjParTypeFile);
   env->SetObjectArrayElement(jobjOBJArr, 5, jobjParDirIN);
   env->SetObjectArrayElement(jobjOBJArr, 6, env->NewStringUTF(FinalOUT));
   env->SetObjectArrayElement(jobjOBJArr, 7, jobjParTypeFile);
   env->SetObjectArrayElement(jobjOBJArr, 8, jobjParDirINOUT);

   Execute(PostOp, 3, 0, 1, 0, newCFG, DimemasOUT, FinalOUT, FinalOUTenv->CallVoidMethod(jobjIT, midExecute,  env->NewStringUTF("Matmul"),  env->NewStringUTF("matmul"), 3, jobjOBJArr);
);
